...
function renderStepIcon(step: StepItem, index: number, p: StepsProps): string {
  const sz = p.iconSize ?? 40
  if (step.icon && (step.icon.startsWith('http') || step.icon.startsWith('/'))) {
    return `<img src="${escapeAttr(step.icon)}" alt="" width="${sz}" height="${sz}" border="0" style="display:block;width:${sz}px;height:${sz}px;border-radius:50%;">`
  }
  const content = escapeHtml(step.icon || String(index + 1).padStart(2, '0'))
  return `<div style="display:inline-block; width:${sz}px; height:${sz}px; border-radius:50%; background-color:${p.numberBackgroundColor}; color:${p.numberColor}; font-family:${fontStack(p.stepTitleFontFamily)}; font-size:${Math.round(sz * 0.4)}px; font-weight:800; line-height:${sz}px; text-align:center; mso-padding-alt:0;">${content}</div>`
}
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="width:100%;">
      ...
      <tr>
        <td align="center" style="padding:0 8px;">
          <div style="font-family:${fontStack(p.stepTitleFontFamily)}; font-size:${p.stepTitleFontSize}px; font-weight:700; color:${p.stepTitleColor}; margin-bottom:6px; line-height:${p.stepTitleLineHeight ?? 1.3}; mso-line-height-rule:exactly;">${escapeHtml(step.title)}</div>
          <div style="font-family:${fontStack(p.descriptionFontFamily)}; font-size:${p.descriptionFontSize}px; color:${p.descriptionColor}; line-height:${p.descriptionLineHeight ?? 1.55}; mso-line-height-rule:exactly;">${escapeHtml(step.description)}</div>
        </td>
      </tr>
    </table>
  </td>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
        <tr>
          ...
          <td valign="top" style="padding-top:8px;">
            <div style="font-family:${fontStack(p.stepTitleFontFamily)}; font-size:${p.stepTitleFontSize}px; font-weight:700; color:${p.stepTitleColor}; margin-bottom:4px; line-height:${p.stepTitleLineHeight ?? 1.3}; mso-line-height-rule:exactly;">${escapeHtml(step.title)}</div>
            <div style="font-family:${fontStack(p.descriptionFontFamily)}; font-size:${p.descriptionFontSize}px; color:${p.descriptionColor}; line-height:${p.descriptionLineHeight ?? 1.55}; mso-line-height-rule:exactly;">${escapeHtml(step.description)}</div>
          </td>
        </tr>
      </table>
  ...
  const titleRow = p.showSectionTitle && p.sectionTitle
    ? `<tr>
        <td align="center" style="padding-bottom:32px;">
          <h2 style="margin:0; font-family:${fontStack(p.sectionTitleFontFamily)}; font-size:${p.sectionTitleFontSize}px; font-weight:700; color:${p.sectionTitleColor};">${escapeHtml(p.sectionTitle)}</h2>
        </td>
       </tr>`
    : ''
