  ...
  const rowsHtml = rows.map(row => {
    const cells = row.map((img, i) => {
      ...
      const content = img.url
        ? `<img class="mobile-img" src="${img.url}" alt="${img.alt}" width="${imgWidth}" height="${imgWidth}" style="display:block; width:${imgWidth}px; height:${imgWidth}px; object-fit:cover;" border="0">`
...
<tr>
  <td class="mobile-padding" bgcolor="${p.backgroundColor}" style="background-color:${p.backgroundColor}; padding:${p.paddingTop}px 40px ${p.paddingBottom}px; text-align:center;">
    <h3 style="margin:0 0 8px; font-family:${fontStack(p.titleFontFamily)}; font-size:26px; font-weight:700; color:${p.titleColor};">${p.title}</h3>
    <p style="margin:0 0 24px; font-family:${fontStack(p.titleFontFamily)}; font-size:14px; color:#9ca3af;">${p.subtitle}</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:24px;">
      ${rowsHtml}
    </table>
    <a href="${p.ctaUrl}" style="display:inline-block; background-color:${p.ctaBackgroundColor}; color:${p.ctaTextColor}; font-family:${fontStack(p.titleFontFamily)}; font-size:15px; font-weight:700; padding:13px 32px; border-radius:6px; text-decoration:none;">${p.ctaText}</a>
  </td>
</tr>`
}
