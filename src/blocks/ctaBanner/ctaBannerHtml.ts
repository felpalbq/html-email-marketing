...
export function ctaBannerHtml(p: CtaBannerProps): string {
  ...
  const badge = p.badge
    ? `<div style="margin-bottom:12px;"><span style="display:inline-block; background-color:${p.badgeColor}; color:${p.badgeTextColor}; font-size:10px; font-weight:700; letter-spacing:2px; text-transform:uppercase; padding:4px 12px; border-radius:20px;">${escapeHtml(p.badge)}</span></div>`
    : ''
  ...
  const textBlock = `
    ...
    ${p.ctaText ? `<a href="${escapeSafeUrl(p.ctaUrl || '#')}" style="display:inline-block; background-color:${p.ctaBackgroundColor}; color:${p.ctaTextColor}; font-family:${fontStack(p.headlineFontFamily)}; font-size:${ctaFontSize}px; font-weight:${ctaFontWeight}; padding:${ctaPaddingV}px ${ctaPaddingH}px; border-radius:${p.ctaBorderRadius}px; text-decoration:none; mso-padding-alt:${ctaPaddingV}px ${ctaPaddingH}px;">${escapeHtml(p.ctaText)}</a>` : ''}`

  const img = p.image
    ? `<img class="mobile-img" src="${escapeAttr(p.image)}" alt="${escapeAttr(p.imageAlt || '')}" width="${imageWidth}" style="display:block; width:${imageWidth}px; height:auto; border-radius:8px;" border="0">`
    : `<div style="width:100%; height:200px; background-color:rgba(255,255,255,0.1); border-radius:8px;"></div>`
...
<tr>
  ...
  const imgTd = `<td class="mobile-col mobile-top-gap" valign="middle" align="${imageAlignment}" width="${imageWidth + 20}" style="width:${imageWidth + 20}px; padding-bottom:16px;">${img}</td>`
